...
### Added
- **Config-based validation rules & `_validation_status`**: attributes can now declare soft validation rules — `min`/`max`, `char_class: alpha|alnum` (Unicode-aware), `required_if` (value- or presence-scoped), `forbidden_if_absent`, and `compare` (cross-field `gt`/`gte`/`lt`/`lte`/`eq`/`ne`) — alongside the existing hard `required`/`type`/`unique` constraints. Unlike those, a rule violation never blocks a write: `POST`/`PUT` on `/api/records/{schema}/{obj}` returns `422` with `confirmation_required: true` and a `validation_rule_violations` list; resubmitting with `?confirm_validation_override=true` (and optional `?override_reason=`) saves the record with a new `_validation_status` (`valid`/`invalid`) system column and a `VALIDATION_OVERRIDE` audit event. `_validation_status` is a computed system column — a value supplied directly in the request body is silently ignored, never persisted verbatim. Bulk import (both plain and `?upsert_key=` upsert) and inbound webhook pushes never hard-fail a row/push over a rule violation — every row/record is written and flagged instead; the import response gains `imported` and `rows` fields. Cross-field rules (`compare`, `required_if`, `forbidden_if_absent`) are evaluated against the full merged record on update, so editing only one side of a compared or dependent pair is still checked correctly against the record's existing other-side value. Publishing a draft recomputes `_validation_status` fresh against the promoted data (for both a brand-new `requires_draft` record and a draft-of-an-existing-active-record) and is never blocked by it, even when the record remains invalid — status is informational, visible on the record everywhere it's shown (list view, detail view, Pending Drafts). Config-time validation rejects `required_if.field`/`compare.field`/`forbidden_if_absent` references to non-existent attributes, an invalid `compare.op`, or an invalid `char_class`. Built with Cerberus; see `docs/reference.md`'s new "Validation Rules" section for the full config syntax, examples, and API contract, and `docs/known_issues.md`'s Design Decisions section for what's intentionally deferred and why (fixed allowed-value lists; cross-object `reference_state` checks); 21 new tests in `tests/test_validation_rules.py` cover the full rule catalog plus the create/update/publish/import/upsert-import/inbound-push wiring and the config-load-time checks
- **Record create/update form now handles validation-rule confirmation**: the New/Edit record form shows a persistent banner listing violated rules with "Save anyway" (resubmits with `confirm_validation_override=true`) and "Cancel" buttons, instead of the plain error message a caller would otherwise see with no way to proceed; covered by two new browser tests in `tests/browser/test_validation_rules.py`

### Fixed
- **Parent-relationship field silently dropped on every create/edit form submission**: the `<select>` for an object's `parent:` relationship (e.g. `_product_category_id` on a `product` with `parent: product_category`) built its `name` attribute as `_product_category_id_id` — a duplicated `_id` suffix — because `loadRecordForm()` in `app.js` set `isParent: true` as a sibling of the field's config object instead of nested inside it, so every check of `v.isParent` silently evaluated to `undefined`/falsy. The browser therefore submitted a key `_filter_columns` didn't recognize as a real column, and it was dropped without error — regardless of whether any validation rule was involved. This is a **regression introduced in v0.6.1** (2026-06-06, the UI facelift that rewrote the record form into `app.js`) — the parent field was built correctly, as its own dedicated code path, in every version from v0.1.0 through v0.6.0, and has been broken in every released version since (v0.6.1 through v0.7.4) until now. Found via manual testing of the validation-rules feature above, which is what first required actually filling in a parent field on a form exercising the new confirm/override flow. New browser tests `test_create_record_with_parent_saves_parent_reference` (`tests/browser/test_records.py`) and `test_edit_form_preselects_parent_reference` (`tests/browser/test_edit_delete.py`) submit the real HTML form for an object with a `parent:` relationship and assert the FK persists — closing a real coverage gap, since every prior parent-FK test posted straight to the JSON API and never exercised the form's field-naming logic at all
- **Type-coercion failures on create/update could surface as an uncaught 500 instead of a clean 422**: `_filter_columns()` in `app/api/objects.py` silently kept the raw, wrong-type value whenever coercing a submitted value to its column's Integer/Numeric/DateTime type failed (e.g. a non-numeric string into a `numeric` attribute), rather than rejecting it — the bad value then reached the database, which could raise an unhandled `sqlalchemy.exc.DataError` instead of the expected `422`. Found while building the validation-rules feature above, which depends on this exact case failing cleanly and unconditionally (a real type violation is a hard constraint and must never be overridable by `confirm_validation_override`). Coercion failures now raise a `_TypeCoercionError` that both `create_record` and every `update_record` branch catch and turn into a plain `HTTPException(422, ...)`, before any database write is attempted
- **Validation rules could crash a write with an uncaught 500 instead of flagging it**: `record_violations()` documents that it never raises, but `compare` raised `TypeError` when one side was a timezone-aware timestamp (`"2026-06-01T00:00:00Z"`) and the other a naive `date` column value — the ordinary case when an API client sends an ISO timestamp while editing only one side of an `end_date`/`start_date` pair — and `char_class` raised `AttributeError` when a string attribute received a native JSON number (`{"code": 12345}`). Both surfaced as a 500 with no override path. Naive and aware datetimes are now compared as UTC, `char_class` evaluates the value as text, and a pair that genuinely can't be ordered is reported as a violation. Config load now also rejects a `compare` between incompatible attribute types (`integer` and `numeric` are compatible) and a `char_class` on a non-text attribute, since neither could ever be evaluated. Found by the pre-v0.8.0 security review; regression tests in `tests/test_validation_rules.py`
- **Revert left `_validation_status` stale**: reverting a record restored an earlier version's values but kept whatever `_validation_status` the record had before, so a record restored to rule-violating values could still read `valid` (and the reverse). Revert is one more write path for the validation-rules feature — alongside create, update, publish, import, upsert and inbound — and now recomputes the status like publish does: informational, never blocking. Test in `tests/test_validation_rules.py`
- **Inbound pushes and JSON imports mishandled native JSON values**: `_coerce_value` assumed every value was a string, so a JSON `true` for a boolean attribute raised an uncaught `AttributeError` — a 500 on `POST /api/inbound/...` and a cryptic per-row error on a JSON import — and a non-string date raised `TypeError`. Booleans, numbers and datetimes are now accepted as-is, and an inbound push containing a value that cannot be coerced to its column type (e.g. `"abc"` for a numeric attribute) answers `422` instead of `500`. Tests in `tests/test_api_import_export.py` and `tests/test_validation_rules.py`
- **`docs/reference.md` listed `date` as `TIMESTAMP WITH TIME ZONE`**; the column is `TIMESTAMP` without a time zone
...
- **Re-added `httpx2` pinned to a patched release**: removing `httpx2` above left `starlette.testclient` (used by nearly every test via FastAPI's `TestClient`) falling back to plain `httpx` with a `StarletteDeprecationWarning` on every run, since the installed Starlette prefers `httpx2` and only warns-and-falls-back when it's absent. Re-checking PyPI's per-release vulnerability data (rather than assuming the single version known at the time of the removal above was still the only one) showed `httpx2` has since released up to `2.12.0`, fixing all four advisories: `GHSA-h4x7-gw46-3wm6` and `GHSA-pf96-p4fj-6566` in `2.11.0`, `GHSA-8xx6-hgc6-gc2m` in `2.12.0`, and `2.12.0` pins `httpcore2==2.12.0`, past that package's own fix for `GHSA-7mj9-2mp8-4m2p` in `2.10.0`. `httpx2>=2.12.0` added back to the `dev` dependency group; verified empirically, not just by semver assumption — the full test suite (306 backend + 38 browser tests) passes identically with it installed and the deprecation warning no longer appears; see `docs/known_issues.md` for the full investigation
- **Auth state is now read from the database on every request, not from the JWT**: the auth middleware trusted the token's `is_admin` claim, so an admin demoted in the Admin UI kept full admin access until their token expired (up to `TOKEN_EXPIRE_HOURS`, default 8), and a password change — an admin setting a new password, or a reset link being used, typically done because an account was compromised — left every session already open on that account valid. `is_admin` now comes from the `users` table (in the same query that already checked `is_active`), and each token carries a short non-reversible fingerprint of the password hash (`pwv`) that must match the current one, so any password change invalidates the user's existing sessions. Tokens issued before this change have no `pwv` and stay valid until they expire, so nobody is logged out on upgrade. Found by the pre-v0.8.0 security review; tests in `tests/test_api_auth.py`
- **Schema metadata no longer visible to users without access to the schema**: `GET /api/config` returned every schema's full definition to any authenticated user, and the HTML object pages (list, detail, history, new, edit) rendered an object's configuration server-side without checking schema permission, so a user granted access to one schema could read the structure (names, attributes, governance owner/steward, inbound source field maps) of all the others — inconsistent with `GET /api/schemas` and the record APIs, which already enforced it. `GET /api/config` now lists only schemas the user can read, and the HTML pages return 403 (read permission for list/detail/history, write for new/edit), checked before the object lookup so a 404 can't be used to probe which objects exist. Admins are unaffected. Tests in `tests/test_api_permissions.py`
- **Authentication skipped for any path starting with `/static`**: the middleware's public-path check was `path.startswith("/static")` with no trailing slash, so a schema named e.g. `staticdata` had its HTML pages served without authentication. Now matches `/static` and `/static/…` only. Test in `tests/test_api_auth.py`
- **CSV/TSV export no longer carries spreadsheet formulas** (CWE-1236, "CSV injection"): a text value starting with `=`, `+`, `-`, `@`, a tab or a carriage return — which any writer, or an inbound source system, can store — was exported verbatim and would execute as a formula when opened in Excel or Sheets. Such cells are now written with a leading apostrophe, and the apostrophe is removed again on CSV/TSV import so export → edit → re-import round-trips losslessly. JSON export is unchanged and numbers (including negative ones) are unaffected; see `docs/reference.md`. Tests in `tests/test_api_import_export.py`
- **Bumped `anyio` 4.12.1 → 4.14.2** for CVE-2026-63374 (TLS hostname validation with internationalized domain names) and CVE-2026-64847 (undrained stderr pipe in process-pool workers), reported by `pip-audit`. Neither code path is reachable from miniMDM — webhooks use the synchronous `httpx` client and nothing calls `anyio.to_process` — but the dependency audit job fails until it is upgraded
- **DDL guard no longer relies on `assert`**: `TableManager` interpolates a column's `server_default` into `ALTER TABLE`/`UPDATE` statements and guarded it with an `assert`, which is stripped under `python -O`. Replaced by an explicit `ValueError` (`_safe_server_default`); the values are hardcoded constants today, so this closes a latent hole rather than a live one (bandit B101/B608). Identifier validation for config and inbound-key source names also now uses `fullmatch`, so a trailing newline can no longer slip past the `$` anchor
- **Editors could change golden and retired records in place, bypassing draft → publish**: `POST .../revert/{version}` rewrote an *active* (golden) record directly for any user with write permission — an Editor could revert the golden record without a Publisher's approval — and `PUT` (and an upsert import as draft) changed a *retired* record in place, although reverting a retired record already required Publisher. Both now require Publisher (or Admin), matching the existing retired-revert rule; Editors can still revert **drafts** and edit normally (which creates a draft). Routing these through draft-copy instead — so Editors keep revert and a Publisher approves the result — is logged in `docs/known_issues.md` as a future feature. **Behaviour change:** an Editor who could previously revert an active record will now get `403`. Tests in `tests/test_api_permissions.py`
- **Import row errors no longer expose the SQL statement**: a failed row in a bulk import returned the raw database exception, including the full `INSERT` statement and every bound parameter (`[SQL: INSERT INTO test.company (...)] [parameters: {...}]`), to any user allowed to import. Rows now report only the database's one-line message (e.g. "A record with this value already exists"). Test in `tests/test_api_import_export.py`
- **Inbound `match_key` claims are now audited**: when an inbound push matched an existing record by `match_key`, the record's `_source_system`/`_source_id` were rewritten by a bare `UPDATE` — on an active (golden) record too — leaving no history version and no audit entry for a provenance change. The claim now writes both, attributed to `inbound:<source>`. Test in `tests/test_api_inbound.py`

### Docs
- **`docs/testing.md` updated for the new validation-rules tests**: `tests/test_validation_rules.py` and `tests/browser/test_validation_rules.py` added to the test suite overview table, with "what this file covers" sections matching the existing per-file convention
- **Pre-v0.8.0 security review outcomes recorded**: `docs/known_issues.md` lists the Low-severity findings deliberately left for later (unvalidated JSON bodies on the reset/admin endpoints, plaintext reset tokens, the incomplete webhook SSRF guard, public `/docs`, and others) the decision to log — not enforce — the default `SECRET_KEY`, and the future feature of routing revert/retired-edit through draft-copy; `docs/reference.md` documents the live auth-state checks, the `/api/config` visibility rule, CSV/TSV export neutralisation, and the `compare`/`char_class` type constraints, and corrects the stated `TOKEN_EXPIRE_HOURS` default (8, not 24); `docs/testing.md` covers the new tests; `docs/docker-setup.md` no longer claims "no default credentials exist" (the `.env.docker` template ships `ADMIN_PASSWORD=admin`) or that PostgreSQL is only exposed on localhost (it is published on all host interfaces), and explains how to bind it to loopback
